...
public class MapCountryCircle extends Component {
    ...
    public MapCountryCircle(int offsetX, int offsetY, int x, int y, double scaling, int armies, Color c) {
        this.x = offsetX + (int) Math.floor(x * scaling);
        this.y = offsetY + (int) Math.floor(y * scaling);
        this.diam = (int) Math.floor(30 * scaling);
        this.c = c;
        this.armies = armies;

        this.setBounds(this.x - this.diam / 2, this.y - this.diam / 2, this.diam, this.diam);
    }
